...
void Fish::move()
{
	...
	coord.x = min((float) 1, coord.x);
	coord.x = max((float) -1, coord.x);
	coord.y = min((float) 0.7, coord.y);
	coord.y = max((float) -1, coord.y);
}
...
bool Fish::insideRangeVision(Fish neighbor)
{
	...
	return (difference <= (angleRangeVision / 360.0) * M_PI);
}
