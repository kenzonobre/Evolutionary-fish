...
#define boidSize 20
#define taxOfMutation 2 // (in percentage)
...
void elitism(int idBestBoid)
{
	...
	for(int i = 0; i < (int) boids.size(); i++)
	{
		...
		{
			newBoids[i] = generateRandomBoid(boidSize);
			newBoids[i].r = boids[i].r;
			newBoids[i].g = boids[i].g;
			newBoids[i].b = boids[i].b;
			newBoids[i].alignmentWeigh = boids[i].alignmentWeigh;
			newBoids[i].cohesionWeigh = boids[i].cohesionWeigh;
			newBoids[i].separationWeigh = boids[i].separationWeigh;
			newBoids[i].avoidSharkWeigh = boids[i].avoidSharkWeigh;
		}
		else
			newBoids[i] = crossOver(boids[i], boids[idBestBoid]);
...
void randomPredation()
{
	...
	for(int i = 1; i < (int) boids.size(); i++)
		if(getAverageFitness(i) < getAverageFitness(idWorstBoid))
			idWorstBoid = i;

	boids[idWorstBoid] = generateRandomBoid(boidSize);
	printf("generation = %d idWorstBoid = %d\n", generation, idWorstBoid);

	for(int k = 0; k < 10; k++)
		boidsFitness[k][idWorstBoid] = 0;

	FILE *file = fopen("chromosome.txt", "a");
	fprintf(file, "generation = %d idWorstBoid = %d\n", generation, idWorstBoid);
	fclose(file);
}
...
void evolve()
{
	...
	for(int i = 0; i < (int) boids.size(); i++)
		boidsFitness[generation % 10][i] += (int) boids[i].fish.size() * 30;
	...
	for(int i = 0; i < (int) boids.size(); i++)
		fprintf(file, "%d ", boidsFitness[generation % 10][i]);
	fprintf(file, "%d id=%d\n", cntDeadFish, idBestBoid);
	fclose(file);

	file = fopen("chromosome.txt", "a");

	for(int i = 0; i < (int) boids.size(); i++)
		fprintf(file, "(id=%d %d %d %d %d)", i, boids[i].alignmentWeigh, boids[i].cohesionWeigh,
										 boids[i].separationWeigh, boids[i].avoidSharkWeigh);
	fprintf(file, "cntDeadFish=%d BestId=%d\n", cntDeadFish, idBestBoid);
	fclose(file);
...
void timer(int)
{
	...
	for(int i = 0; i < (int) boids.size(); i++)
	{
		for(int j = 0; j < (int) boids[i].fish.size(); j++)
			if(boids[i].fish[j].eatenByShark(sharks))
			{
				...
				boidsFitness[generation % 10][i] += timeCounter / 60;
			}
	}
	...
	glutTimerFunc(1000 / simulationSpeed, timer, 0);// Garante que esta função será chamada de novo
}
...
*/
	...
	text = "[q] INCREASE SPEED";
	drawText(text, 0, (1 - 6 * borderSize), color, color, color);

	text = "[a] DECREASE SPEED";
	drawText(text, 0, (1 - 12 * borderSize), color, color, color);

	text = "SPEED = " + to_string(simulationSpeed);
	drawText(text, 0, (1 - 18 * borderSize), color, color, color);
	...
	text = (showAnimation) ? "[s] HIDE ANIMATION" : "[s] SHOW ANIMATION";
	drawText(text, 0.5, (1 - 18 * borderSize), color, color, color);
}
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	if(key == 'q')	simulationSpeed++;
	if(key == 'a')	simulationSpeed--;

	simulationSpeed = min(240, simulationSpeed);
	simulationSpeed = max(30, simulationSpeed);
}
